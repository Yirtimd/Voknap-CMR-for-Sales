...
const canManage = computed(() =>
  crmStore.me.value?.permissions.includes("sales:manage") ||
  ["owner", "admin", "sales_manager"].includes(crmStore.me.value?.role ?? "")
);

function emptyStage(index: number): StageDraft {
  return { name: "", code: `stage_${index + 1}`, probability: 0, stage_type: "open", required_fields: "", is_active: true };
}
...
function edit(pipeline: Pipeline) {
  ...
  form.stages = pipeline.stages.map((stage) => ({
    id: stage.id,
    name: stage.name,
    code: stage.code,
...
async function save() {
  ...
  if (crmStore.error.value) return;
  reset();
}
...
<template>
  <section class="pipeline-settings">
    <p v-if="crmStore.error.value" class="alert error pipeline-message">{{ crmStore.error.value }}</p>
    <p v-if="!canManage" class="alert warning pipeline-message">Воронки доступны для просмотра. Для изменения нужны права управления продажами.</p>
    <form class="panel pipeline-editor" @submit.prevent="save">
      ...
      <fieldset><legend>Этапы</legend>
        <article v-for="(stage, index) in form.stages" :key="stage.id || index" class="stage-row">
          ...
          <span class="stage-activity" :class="{ inactive: !stage.is_active }">{{ stage.is_active ? "Активен" : "Отключён" }}</span>
          <button class="danger-quiet" type="button" :disabled="!canManage || form.stages.length === 1" @click="form.stages.splice(index, 1)">Удалить</button>
        </article>
        <button class="secondary" type="button" :disabled="!canManage" @click="form.stages.push(emptyStage(form.stages.length))">Добавить этап</button>
      </fieldset>
      <button type="submit" :disabled="!canManage || crmStore.isLoading.value">{{ editing ? "Сохранить изменения" : "Создать воронку" }}</button>
    </form>
    <section class="panel pipeline-list"><h2>Воронки продаж</h2>
      <article v-for="pipeline in crmStore.pipelines.value" :key="pipeline.id">
        ...
        <button type="button" @click="edit(pipeline)">{{ canManage ? "Настроить" : "Посмотреть" }}</button>
      </article>
