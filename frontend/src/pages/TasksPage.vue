...
function priorityLabel(priority: string) {
  ...
  return tone === "high" ? "High" : tone === "low" ? "Low" : "Medium";
}
...
const stats = computed(() => [
  ...
  { code: "waiting", icon: "◷", label: "Waiting", value: openTasks.value.filter(isWaiting).length },
...
const groups = computed<TaskGroup[]>(() => {
  ...
  const rows: TaskGroup[] = [
    ...
    { code: "waiting", title: "Waiting", items: open.filter(isWaiting) },
  ...
  <section class="tasks-page">
    ...
    <section class="tasks-toolbar" aria-label="Управление задачами">
      ...
      <select v-model="priorityFilter" class="tasks-select" aria-label="Приоритет">
        <option value="all">Приоритет: Умный</option>
    ...
    <section class="tasks-shell">
      <main class="tasks-main">
        <section v-if="aiFocus.length" class="tasks-ai-focus">
          <header><div><span class="ai-spark" aria-hidden="true">✦</span><h2>AI Focus</h2><small>Рекомендуем</small></div></header>
      ...
      <aside class="tasks-right-rail">
        <section class="tasks-rail-card">
          ...
          <div class="rail-list">
            <article><span class="rail-icon purple">✉</span><div><strong>Follow-up</strong><small>Проверьте задачи без ответа</small></div><b>{{ openTasks.filter((task) => /ответ|письм|follow/i.test(`${task.title} ${task.description}`)).length }}</b></article>
    ...
    <details ref="createDrawer" class="tasks-create-drawer">
      ...
      <div class="drawer-head"><div><small>Execution</small><h2>Новая задача</h2></div><button type="button" aria-label="Закрыть" @click="closeCreateDrawer">×</button></div>
      <form class="tasks-create-form" @submit.prevent="crmStore.createTask().then(closeCreateDrawer)">
        ...
        <label><span>Приоритет</span><select v-model="crmStore.taskForm.value.priority"><option value="high">High</option><option value="normal">Medium</option><option value="low">Low</option></select></label>
